...
API_KEY = os.getenv("API_KEY", "")  # 빈 문자열이면 API 키 검증을 건너뜀
...
async def verify_api_key(x_api_key: Optional[str] = Header(None)):
    # API 키가 설정되어 있고, 요청의 API 키가 일치하지 않을 때만 오류 발생
    if API_KEY and x_api_key != API_KEY:
        logger.warning(f"잘못된 API 키 시도: {x_api_key[:5] if x_api_key else 'None'}...")
...
async def mcp_rpc_endpoint(request: Request):
    ...
    API_KEY가 빈 문자열이면 인증을 건너뜁니다 (MVP 단계용)
    """
    try:
...
async def root():
    ...
    서버 기본 정보를 제공합니다.
    """
    auth_info = "API 키 필요" if API_KEY else "인증 필요 없음 (MVP 모드)"
    return {
...
def parse_args():
    ...
    parser.add_argument("--host", type=str, default=HOST,
                        help=f"서버 호스트 (기본값: {HOST})")
    parser.add_argument("--port", type=int, default=PORT,
                        help=f"서버 포트 (기본값: {PORT})")
    parser.add_argument("--tools", type=str,
                        help="활성화할 도구 목록 (쉼표로 구분)")
    parser.add_argument("--list-tools", action="store_true",
                        help="사용 가능한 도구 목록 표시")
    parser.add_argument("--metadata", type=str, default=METADATA_FILE,
                        help=f"메타데이터 파일 경로 (기본값: {METADATA_FILE})")
    parser.add_argument("--stylebook-dir", type=str, default=STYLEBOOK_DIR,
                        help=f"스타일북 디렉토리 경로 (기본값: {STYLEBOOK_DIR})")
    parser.add_argument("--no-auth", action="store_true",
                        help="API 키 인증을 비활성화 (MVP 모드)")
...
def main():
    ...
    if args.tools:
        tool_list = args.tools.split(',')
        # 유효한 도구만 필터링
        valid_tools = {k: v for k, v in available_tools.items() if k in tool_list}
        if not valid_tools:
            print("오류: 유효한 도구가 지정되지 않았습니다.")
            print("사용 가능한 도구 목록:")
            for tool_name in available_tools:
                print(f"  - {tool_name}")
            return
    ...
    # 환경 변수 설정
    global HOST, PORT, METADATA_FILE, STYLEBOOK_DIR, API_KEY
    HOST = args.host
    PORT = args.port
    METADATA_FILE = args.metadata
    STYLEBOOK_DIR = args.stylebook_dir
    
    # --no-auth 옵션이 있으면 API 키를 비활성화
    if args.no_auth:
        API_KEY = ""
        print("MVP 모드: API 키 인증이 비활성화되었습니다.")
    
    # API 키 상태 출력
    auth_mode = "API 키 인증 활성화됨" if API_KEY else "MVP 모드: API 키 인증 비활성화"
    
    # 서버 실행
    import uvicorn
    print(f"서버 시작: {HOST}:{PORT}")
    print(f"인증 모드: {auth_mode}")
    print(f"활성화된 도구: {', '.join(available_tools.keys())}")
    uvicorn.run("stylebook_mcp_server.server:app", host=HOST, port=PORT, reload=True)
